...
# Included from x86/_types.h

# Included from machine/endian.h

# Included from x86/endian.h
_QUAD_HIGHWORD = 1
_QUAD_LOWWORD = 0
_LITTLE_ENDIAN = 1234
_BIG_ENDIAN = 4321
...
def __bswap16_gen(x): return (__uint16_t)((x) << 8 | (x) >> 8)

def __bswap32_gen(x): return \

def __bswap64_gen(x): return \

def __bswap16(x): return \

def __bswap32(x): return \

def __bswap64(x): return \

def __bswap16(x): return __bswap16_var(x)

def __bswap32(x): return __bswap32_var(x)

def __bswap64(x): return __bswap64_var(x)
